...
void test_print_need_info(ISession *session)
{
    IConsole *console = NULL;
    HRESULT rc;
    ...
    if (SUCCEEDED(rc) && console)
    {
        IMachineDebugger* debugger = NULL;
        PRBool sstep = PR_FALSE;
        ...
        if (SUCCEEDED(rc) && debugger)
        {
            ...
            if (FAILED(rc))
            {
                printf("[Error test_print_need_info] IMachineDebugger_LoadPlugIn() Failed.\n");
                PrintErrorInfo("server.c", "IMachineDebugger_LoadPlugIn", rc);
            }

            rc = IMachineDebugger_get_SingleStep(debugger, &sstep);
            if (FAILED(rc))
            {
                printf("[Error test_print_need_info] IMachineDebugger_get_SingleStep() Failed.\n");
                PrintErrorInfo("server.c", "IMachineDebugger_get_SingleStep", rc);
            }
            printf(sstep ? "true\n" : "false\n");

            sstep = PR_TRUE;
            rc = IMachineDebugger_put_SingleStep(debugger, sstep);
            if (FAILED(rc))
            {
                printf("[Error test_print_need_info] IMachineDebugger_put_SingleStep() Failed.\n");
                PrintErrorInfo("server.c", "IMachineDebugger_put_SingleStep", rc);
            }


            BSTR set_reg_name;
            BSTR set_reg_value;
            g_pVBoxFuncs->pfnUtf8ToUtf16("eax", &set_reg_name);
            g_pVBoxFuncs->pfnUtf8ToUtf16("0xabcd", &set_reg_value);
            rc = IMachineDebugger_SetRegister(debugger, 0, set_reg_name, set_reg_value);
            if (FAILED(rc))
            {
                printf("[Error test_print_need_info] IMachineDebugger_SetRegister() Failed.\n");
                PrintErrorInfo("server.c", "IMachineDebugger_SetRegister", rc);
            }

            rc = IMachineDebugger_GetRegisters(debugger, 0, ComSafeArrayAsOutTypeParam(reg_name_s, BSTR), ComSafeArrayAsOutTypeParam(reg_value_s, BSTR));
            if (SUCCEEDED(rc))
            {
                BSTR *reg_name = NULL;
                BSTR *reg_value = NULL;
                ...
                {
                    char *name, *value;
                    g_pVBoxFuncs->pfnUtf16ToUtf8(reg_name[i], &name);
                    if (name[0] == '\0') continue;
                    g_pVBoxFuncs->pfnUtf16ToUtf8(reg_value[i], &value);
                    printf("[registers] %s: %s\n", name, value);
                    g_pVBoxFuncs->pfnUtf8Free(name);
                    g_pVBoxFuncs->pfnUtf8Free(value);
                }
            ...
            {
                printf("[Error test_print_need_info] IMachineDebugger_GetRegisters() Failed.\n");
                PrintErrorInfo("server.c", "IMachineDebugger_GetRegisters", rc);
            }

            SAFEARRAY* mem_arr = NULL;
            u_int8_t in_array[] = {0x12, 0x13, 0x1, 0x3, 0x5, 0x20, 0xcf, 0x38, 0xdd, 0xae, 0x55, 0x14, 0xee, 0x34, 0xa2, 0x77, 0x4e, 0xac};
            u_int8_t* out_array = NULL;

            mem_arr = g_pVBoxFuncs->pfnSafeArrayCreateVector(VT_UI1, 0, 0x10);
            g_pVBoxFuncs->pfnSafeArrayCopyInParamHelper(mem_arr, in_array, 0x10);
            rc = IMachineDebugger_WritePhysicalMemory(debugger, 0x1000, 0x10, ComSafeArrayAsInParam(mem_arr));
            if (FAILED(rc))
            {
                printf("[Error test_print_need_info] IMachineDebugger_WritePhysicalMemory() Failed.\n");
                PrintErrorInfo("server.c", "IMachineDebugger_WritePhysicalMemory", rc);
            }
            g_pVBoxFuncs->pfnSafeArrayDestroy(mem_arr);

            mem_arr = g_pVBoxFuncs->pfnSafeArrayOutParamAlloc();
            rc = IMachineDebugger_ReadPhysicalMemory(debugger, 0x1000, 0x10, ComSafeArrayAsOutTypeParam(mem_arr, u_int8_t));
            if (SUCCEEDED(rc))
            {
                ULONG arr_sz = 0;
                ULONG i, arr_len;
                g_pVBoxFuncs->pfnSafeArrayCopyOutParamHelper((void **)&out_array, &arr_sz, VT_UI1, mem_arr);
                g_pVBoxFuncs->pfnSafeArrayDestroy(mem_arr);
                arr_len = arr_sz / sizeof(out_array[0]);

                printf("outlen: %u\n", arr_len);
                for (i = 0; i < arr_len; i++)
                {
                    printf("%02x\t", out_array[i]);
                }
                printf("\n");
                g_pVBoxFuncs->pfnArrayOutFree(out_array);
            }
            else
            {
                printf("[Error test_print_need_info] IMachineDebugger_ReadPhysicalMemory() Failed.\n");
                PrintErrorInfo("server.c", "IMachineDebugger_ReadPhysicalMemory", rc);
            }
            ...
            u_int8_t vin_array[] = {0xaa, 0xbb, 0xd1, 0x33, 0x65, 0x10, 0xde, 0x32, 0xba, 0xae, 0x55, 0x14, 0xee, 0x34, 0xa2, 0x77, 0x4e, 0xac};
            u_int32_t cpu_id = 0;

            mem_arr = g_pVBoxFuncs->pfnSafeArrayCreateVector(VT_UI1, 0, 0x10);
            g_pVBoxFuncs->pfnSafeArrayCopyInParamHelper(mem_arr, vin_array, 0x10);
            rc = IMachineDebugger_WriteVirtualMemory(debugger, cpu_id, 0x1000, 0x10, ComSafeArrayAsInParam(mem_arr));
            if (FAILED(rc))
            {
                printf("[Error test_print_need_info] IMachineDebugger_WriteVirtualMemory() Failed.\n");
                PrintErrorInfo("server.c", "IMachineDebugger_WriteVirtualMemory", rc);
            }
            g_pVBoxFuncs->pfnSafeArrayDestroy(mem_arr);

            mem_arr = g_pVBoxFuncs->pfnSafeArrayOutParamAlloc();
            rc = IMachineDebugger_ReadVirtualMemory(debugger, cpu_id, 0x1000, 0x10, ComSafeArrayAsOutTypeParam(mem_arr, u_int8_t));
            if (SUCCEEDED(rc))
            {
                ULONG arr_sz = 0;
                ULONG i, arr_len;
            ...
            {
                printf("[Error test_print_need_info] IMachineDebugger_ReadVirtualMemory() Failed.\n");
                PrintErrorInfo("server.c", "IMachineDebugger_ReadVirtualMemory", rc);
            }
            ...
            printf("[Error test_print_need_info] IConsole_get_Debugger() Failed.\n");
...
static void startVM(const char *argv0, IVirtualBox *virtualBox, ISession *session, BSTR id)
{
    ...
    if (SUCCEEDED(rc))
    {
        ...
        else
        {
            ...
            // init_dbgapi(session);
            // release_dbgapi();
            // test_print_need_info(session);
            dbgshell_start(progress, machine, session, vbox, vboxclient);
        }
