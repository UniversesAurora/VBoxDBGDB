...
/** virtual memory **/
VMMDECL(int) VMR3PhysSimpleReadGCPtr(PVMCPUCC pVCpu, void *pvDst, RTGCPTR GCPtrSrc, size_t cb)
{
    return PGMPhysSimpleReadGCPtr(pVCpu, pvDst, GCPtrSrc, cb);
}

VMMDECL(int) VMR3PhysSimpleWriteGCPtr(PVMCPUCC pVCpu, RTGCPTR GCPtrDst, const void *pvSrc, size_t cb)
{
...
VMMR3DECL(bool) VMR3GetAttached(PUVM pUVM)
{
    return pUVM->pVM->dbgf.s.fAttached;
}

VMMDECL(int) VMR3PhysGCPtr2GCPhys(PVMCPUCC pVCpu, RTGCPTR GCPtr, PRTGCPHYS pGCPhys)
{
    return PGMPhysGCPtr2GCPhys(pVCpu, GCPtr, pGCPhys);
}
